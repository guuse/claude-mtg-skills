...
| **mtg-std-build** | Builds a **meta-relevant, cohesive 60-card Standard deck for MTG Arena** by **starting from real, current tournament decklists pulled from mtgtop8.com** (the one bot-fetchable meta source — blocked sites are never scraped) and adapting a proven list to your centerpiece/archetype, the live ladder meta, your collection, and budget — instead of brewing a 60 from scratch. A pet card is **grafted into the proven shell that best supports it**. Verifies Standard legality, rarity, and Arena availability via Scryfall, and costs the deck in **Arena wildcards** against a budget tier (1–5) and your owned collection. Produces an **annotated decklist** (roles, rarity per card, mana curve, wildcard-cost breakdown, meta plan) and an **Arena import list** ready to paste in-client. Supports BO1 ladder and BO3 + sideboard. **Every finished deck ships with a ★ rating** (ladder-and-tier scorecard in `deck.md`). |
